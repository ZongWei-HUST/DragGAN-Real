...
def init_images(global_state, is_real_w=None):
    ...
    if not is_real_w:
        ...
        w_pivot = torch.load(is_real_w)
        state['pretrained_weight'] = "stylegan2_custom_image"
...
# init_pkl = 'stylegan2_custom_llz1'
...
with gr.Blocks() as app:
    ...
    with gr.Row():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                # ----------------------------
                # Drag your own image here 
                # ----------------------------
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='Drag Real', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        with gr.Row():
                            upload_image_path = gr.Image(type='filepath')
                        with gr.Row():
                            max_pti_steps = gr.Number(value=350, label="Max PTI steps")
                        with gr.Row():
                            click_align_pti = gr.Button(value="Align Face & PTI Inversion")
    ...
    def on_click_align_pti(raw_image_path, max_pti_steps, global_state):
        cur_path = os.getcwd()
        os.chdir(os.path.join(cur_path, 'PTI'))
        
        # Align Face & PTI Inversion & Get .pkl
        max_pti_steps = int(max_pti_steps)
        os.system(f"python pti_run.py --raw_img_path {raw_image_path} --max_pti_steps {max_pti_steps}")
        os.system(f"mv ./results/checkpoints/stylegan2_custom_image.pkl ../checkpoints")
        os.system(f"mv ./results/embeddings/embedding.pt ../checkpoints")
        ...
        form_pretrained_dropdown.update(choices=list(valid_checkpoints_dict.keys()))
        form_pretrained_dropdown.update(value="stylegan2_custom_image")
        global_state['pretrained_weight'] = form_pretrained_dropdown.value
   
        global_state = init_images(global_state, is_real_w="./checkpoints/embedding.pt")
        return global_state, global_state["images"]['image_show']

    click_align_pti.click(
        on_click_align_pti,
        inputs=[upload_image_path, max_pti_steps, global_state],
...
app.launch(debug=args.debug, server_name="0.0.0.0" if args.listen else "127.0.0.1")
