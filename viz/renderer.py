    ...
    def _render_drag_impl(self, res,
        ...
        **kwargs
        ...
        if is_drag:
            ...
            feat_resize = F.interpolate(feat[feature_idx], [h, w], mode='bilinear', align_corners=True)
            if self.feat_refs is None:
                self.feat0_resize = F.interpolate(feat[feature_idx].detach(), [h, w], mode='bilinear', align_corners=True)
                self.feat_refs = []
                for point in points:
